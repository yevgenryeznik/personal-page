...
Let $\theta$ be a parameter of interest. Somehow, a prior information about $\theta$ is available (historical reports, similar experiments, subjective estimates, etc. ). We can represent this information in terms of prior distribution

$$
  p(\theta)
$$

Then, we perform an experiment where we observe a random variable $Y=y$. Bayes' formula tells that
...
$$
  p(y)=\sum\limits_{\theta}p(\theta)p(y|\theta) \text{ (discrete case)} 
$$

or

$$
  p(y)=\int\limits_{\Theta}p(\theta)p(y|\theta)d\theta \text{ (continuous case)}
$$
...
What about the word “radom”? It could be:
...
$$
  \text{Pr}(\theta|Y ="radom")\propto p(\theta)\text{Pr}(Y="radom"|\theta).
$$
Consider only three possibilities for the intendent word, $\theta$
(“random”, “radon”, “radom”). 

Posterior probability of interest, e.g. for $\theta_1$="random":

$$
\text{Pr}(\theta_1 |"radom") = \frac{p(\theta_1)\text{Pr}("radom"|\theta_1 )}{\sum_{j=1}^3p(\theta_j )\text{Pr}("radom"|\theta_j)}.
$$
...
**Remark**. Radom is a city in Poland 
...
- _**Bayesian**_. Uncertainty of $p_B$ is modelled with a p.d.f. $f(p)$. Then
$$
  \text{Pr}^\text{pred}(B)=\mathbf{E}[p]=\int pf(p)dp.
$$

_**The crucial point in Bayesian methodology is the choice of the density $f$. The density reflects all our knowledge about the studied problem**_.
...
$$
  f^\text{prior}(\theta)
$$
- Then a result of experiment is observed: some event $C$ is true.
...
#### Example (severe adverse event (AE) in a clinical trial)

Let us consider a clinical trial where when 1 severe AE out of 3 AEs has been observed, and we are interested in probability $p_B$ of a severe AE. Since we had no information about it before the experiment, we may assume that $p_B\sim U(0,1)$.
$$
  f^\text{prior}(p)=1.
$$
...
Important step in Bayesian modelling: assume that $F(x;\theta)$ is actually the conditional distribution of $x$ given $\theta$, i.e. 
$$
...
- Posterior density: 
$$
f^\text{post}(\theta_1,\theta_2) \sim Dir(\widetilde{a}_1, \widetilde{a}_2, \widetilde{a}_3), \text{ where } \widetilde{a}_ = a_1 + k_1, \widetilde{a}_2 = a_2 + k_2, \widetilde{a}_3 = a_3 + k_3. 
$$
