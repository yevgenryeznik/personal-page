...
Let $X\sim F_X (x)$ and $Y\sim F_Y (y)$ be two random variables the values of which can be observed in a random experiment. The _**joint distribution function**_ of $X$ and $Y$ is defined as
...
In a case of _**discrete random variables**_, a _**joint probability mass function**_ (pmf) is used:
...
Ina clinical trial, two types of adverse events (AEs) can be observed: $X$ and $Y$:
...
tribble(
  ...
  add_header_above(c("$p_{X, Y}(x, y)$" = 5), escape = FALSE)
```
...
Two random variable $X$ and $Y$ have the joint pmf $p_{X, Y}(x, y) = \text{P}(X = x, Y = y)$ is given by the following table:
...
In a case of _**continuous random variables**_, a _**joint probability density function**_ (pdf) $f_{X, Y}(x,y)$ is used, for which:
...
Two electronic components of some device work in harmony for the success of the total system. Let $X$ and $Y$ denote the life in hours of respective component. The joint density of $X$ and $Y$ is
...
\begin{aligned}
  ...
  &\approx 0 0.0008262507
\end{aligned}
$$
```{r marginal-pdfs-example}
...
P.d.f. of this distribution, by Bayes' formula is
...
```{r correlation, echo=FALSE, warning=FALSE, fig.cap="Bivariate normal distribution"}
means <- list(c(0, 1), c(0, 1), c(0, 1), c(0, 1))
sigma <- list(matrix(c(1, 0, 0, 1), 2), matrix(c(1, -0.2, -0.2, 1), 2), matrix(c(1, -0.5, -0.5, 1), 2), matrix(c(1, 0.9, 0.9, 1), 2))
label <- list("no~~correlation", "rho == -0.2", "rho == -0.5", "rho == -0.9")
...
ggplot(df, aes(x = X, y = Y)) +
  geom_point(alpha = .5) +
  geom_density_2d()+
  facet_wrap(~ Z, labeller = label_parsed, scales = "free_y")
